...
| `port->get_peer_cred` (Hurd: nonce lookup in `pending_auth[]` populated by the auth drain's `auth_server_authenticate`) | v0.8 design-2.2 slice 5 rewrite shipped on hurd branch: the old cmsg+recvmsg path is retired; the new body takes the 16-byte rendezvous NONCE the supervisor wrote to the client on accept and looks up the matching row in `pending_auth[]`.  The row is populated by `S_geos_auth_submit_nonce` (the auth-port drain handler, slice 3 surface) which now runs the real 13-arg `auth_server_authenticate` against the rendezvous send right delivered in the submit_nonce mach_msg.  Retry budget 5x200ms; ETIMEDOUT on miss; ENOSYS if `publish_auth_port` has not run.  See `docs/runlogs/2026-05-18-hurd-end-to-end-multi-user.md` | YES on 2026-05-18 (slice-5 PASS marker in `tests/hurd-client-handshake.c` on the Hurd VM: `parent OK pending_auth row uid=<euid> gid=<egid> (real, not sentinel); parent slice5_handshake_ok=1`) |
| `port->client_auth_handshake` (Hurd: rendezvous + `auth_user_authenticate`) | v0.8 design-2.2 slice 4 body lives on hurd branch: allocates a fresh Mach receive port, inserts a MAKE_SEND on it, posts a hand-rolled `mach_msg(msgh_id=GEOS_AUTH_SUBMIT_NONCE_MSGID=90001)` carrying the rendezvous MOVE_SEND + 16-byte nonce against the auth-port send right from `file_name_lookup("/servers/geos-auth")`, then runs `auth_user_authenticate(getauth(), rendez_rcv, MAKE_SEND, &newport)` to complete the rendezvous against the supervisor's matching `auth_server_authenticate` call inside `S_geos_auth_submit_nonce`.  Every exit branch deallocates rendez_rcv, auth_send, newport. | YES on 2026-05-18 (slice 4 PASS marker `child slice4_handshake_ok=1` survived the slice-5 server-side rewrite; receipt `docs/runlogs/2026-05-18-hurd-end-to-end-multi-user.md`) |
