...
| `port->arm_parent_death` (Linux: `prctl(PR_SET_PDEATHSIG)`; Hurd: `MACH_NOTIFY_DEAD_NAME` via `mach_port_request_notification`) | v0.9.8 slot landed on main; Linux body calls `prctl` directly; Hurd body returns ENOSYS as a partial slot, the real `MACH_NOTIFY_DEAD_NAME` watcher-thread implementation is the v0.9.9 follow-on (see receipt for the design); call site is `spawn_xorg()`'s post-fork child, which tolerates ENOSYS as a defence-in-depth gap and logs to /dev/console.  same commit removed the v0.7.x force-console block under `PORT_HURD` and added the `/usr/bin/Xvfb` default for Hurd UI mode (probe C+D confirmed Xvfb 2:21.1.22-1 works) | YES on Linux (prctl shipped, freeze-test `freeze-test-arm-parent-death.el` covers slot-bound + linux-prctl-call + raises-pid1-error-class + error-shape); **PARTIAL on Hurd, live-verified** on 2026-05-21 (v0.9.8 VM-verify on Debian GNU/Hurd 0.9: ENOSYS-class `pid1-error` signalled deterministically across 100 back-to-back calls with VmSize delta 0; C-side range guard catches sig=999 and sig=0 with EINVAL before any port-table dispatch; pid1 boots into `GEOS_MODE_UI` and spawns `/usr/bin/Xvfb` with the documented `is_xvfb` argv; v0.9.5/v0.9.6/v0.9.7 regression sweep clean; runlog `docs/runlogs/2026-05-21-v098-vm-verify.md`.  v0.9.9 real impl pending; probe receipt `docs/runlogs/2026-05-21-hurd-xorg-probe.md`) |
